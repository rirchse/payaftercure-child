<?php
/**
 * template Name: Srf Health Analysis
 */
...
<div class="smartfinderrem" style="text-align:center"><h2>Find Out How Healthy You Are</h2></div>
...
/** if illness selected then show question answer form */
  ...
  // {
    ...
    $illness_id = 18;
    // ( sanitize($_POST['illness_id']) <= 6 || sanitize($_POST['illness_id']) == 22)?(1):(sanitize($_POST['illness_id']));
    ...
    foreach($questions as $key => $question)
    {
      ...
      $hidden_text = ($question->question_shown  == 0) ? (' style="display:no/ne" class="tr' . $question->question_id . '"') : (' class="tr' . $question->question_id . '"');
      ...
      if($question->question_id == 1122 || $question->question_id == 1123)
      {
        ...
        {
          $hidden_text .= ' style="display:no/ne"';
        }
      }
